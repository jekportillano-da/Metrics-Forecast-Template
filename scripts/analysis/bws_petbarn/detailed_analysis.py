"""
BWS and Petbarn Offer Pause Impact Analysis - DETAILED VERSION
============================================================
...
print("=" * 100)
print("BWS AND PETBARN OFFER PAUSE IMPACT ANALYSIS - COMPREHENSIVE REPORT")
print("=" * 100)
...
bws_variants = sorted([m for m in all_merchants if 'BWS' in m.upper()])
petbarn_variants = sorted([m for m in all_merchants if 'PETBARN' in m.upper()])

print(f"\n📋 BWS VARIANTS IDENTIFIED ({len(bws_variants)}):")
...
# ============================================================================
# SECTION 2: BWS DETAILED ANALYSIS
# ============================================================================
print("\n\n" + "█" * 100)
print("SECTION 2: BWS COMPREHENSIVE ANALYSIS")
print("█" * 100)

# Collect BWS data by variant and month
bws_detailed_data = []
bws_monthly_totals = []
...
print("\n" + "─" * 100)
print("2.1 BWS MONTHLY PERFORMANCE SUMMARY")
print("─" * 100)
print("\n" + bws_monthly_df.to_string(index=False))

# Calculate key metrics
print("\n" + "─" * 100)
print("2.2 BWS KEY PERFORMANCE INDICATORS")
print("─" * 100)
...
print("\n" + "─" * 100)
print("2.3 BWS VARIANT-LEVEL PERFORMANCE BREAKDOWN")
print("─" * 100)
...
# Collect Petbarn data by variant and month
...
print("\n" + "─" * 100)
print("4.1 BWS NOVEMBER PROJECTION")
print("─" * 100)

# Multiple projection methods for BWS
...
print("─" * 80)
print(f"{'BWS':<15} {bws_projected:>20,.0f} ${(bws_projected * (total_bws_spend / total_bws_users)):>23,.2f} {(bws_projected/total_projected_users*100):>14.1f}%")
print(f"{'Petbarn':<15} {petbarn_projected:>20,.0f} ${(petbarn_projected * (total_petbarn_spend / total_petbarn_users)):>23,.2f} {(petbarn_projected/total_projected_users*100):>14.1f}%")
print("─" * 80)
...
print(f"\n📊 BWS vs Petbarn Comparison:")
print(f"\n{'Metric':<40} {'BWS':>20} {'Petbarn':>20}")
print("─" * 80)
...
1. 📊 SCALE OF IMPACT
   ...
   • BWS represents {(bws_projected/total_projected_users*100):.1f}% of impact
   • Petbarn represents {(petbarn_projected/total_projected_users*100):.1f}% of impact

2. ⚠️  BWS GRADUAL PAUSE CONTRADICTION
...
4. 💰 REVENUE CONCENTRATION
   • BWS total spend: ${total_bws_spend:,.2f} ({(total_bws_spend/(total_bws_spend+total_petbarn_spend)*100):.1f}%)
   • Petbarn total spend: ${total_petbarn_spend:,.2f} ({(total_petbarn_spend/(total_bws_spend+total_petbarn_spend)*100):.1f}%)
   • Combined 6-month spend: ${total_bws_spend + total_petbarn_spend:,.2f}

5. 🎯 VARIANT DISTRIBUTION
   • BWS operates {len(bws_variants)} variants
   • Petbarn operates {len(petbarn_variants)} variants
   • Top BWS variant: {variant_summary.iloc[0]['variant']} ({variant_summary.iloc[0]['total_spend']/variant_summary['total_spend'].sum()*100:.1f}% of spend)
   • Top Petbarn variant: {variant_summary_pb.iloc[0]['variant']} ({variant_summary_pb.iloc[0]['total_spend']/variant_summary_pb['total_spend'].sum()*100:.1f}% of spend)
""")
...
1. INVESTIGATE BWS SEPTEMBER-OCTOBER PERFORMANCE
...
⚠️  LIMITATIONS:
   ...
   • High volatility (especially Petbarn) reduces forecast confidence
   • May-June data appears limited for some variants

✅ CONFIDENCE LEVELS:
   • BWS Projection: MODERATE (data shows unexpected patterns)
   • Petbarn Projection: LOW-MODERATE (high volatility, outlier spike)
